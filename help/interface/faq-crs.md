...
| **(Somente Adobe Target)** O que a coluna  **** Sincronizar status representa para os arquivos carregados na fonte de atributos do cliente? | O número de registros publicados e sincronizados pelo Adobe Target pode ser visualizado ao selecionar o ícone Sincronizar status em relação a um arquivo de atributo específico. `Sync %` é uma métrica em tempo real que especifica a % de perfis que foram sincronizados no Adobe Target.<br> **Observação:** pode levar até 24 horas para que os atributos sincronizem com o Adobe Target. |
