---
description: Obtenha respostas para perguntas frequentes sobre o  [!DNL Customer Attributes]  no Adobe CX Enterprise, para Adobe Analytics e Adobe Target.
...
| Vários logons no mesmo dispositivo | Ao usar o [!DNL Customer Attributes] para carregar perfis de clientes em uma fonte de dados, a Adobe não recomenda que os usuários compartilhem dispositivos (ou seja, a mesma CX Enterprise ID). A CX Enterprise ID (ECID) persiste no dispositivo. O compartilhamento de dispositivos pode fazer com que a ECID vincule vários usuários à mesma ECID, resultando em resultados inesperados [!DNL Target]. **Observação:** para dispositivos móveis, a ECID é permanente depois que o aplicativo para dispositivos móveis é instalado. Reinstale o aplicativo para gerar uma nova ECID. Na Web, um novo ECID é gerado depois que o cookie do navegador é limpo. |
...
| É necessário migrar para o Serviço da CX Enterprise ID? | A migração depende dos aplicativos que você usa. <ul><li>Adobe Analytics: recomendado </li><li>Adobe Target: obrigatório. </li></ul>O uso do serviço da CX Enterprise ID permite a funcionalidade mais recente do CX Enterprise, incluindo públicos em tempo real, a modernização do Adobe Target, a integração do Analytics e o rastreamento de pulsação de vídeo. |
...
| **(Somente Adobe Target)** Como o [!DNL Customer Attributes] se compara à API de perfil em massa da Adobe Target? | A API de perfil em massa permite que os perfis do Adobe Target sejam atualizados diretamente pela API, para um perfil individual ou em massa. O recurso é semelhante aos [!DNL Customer Attributes], com estas principais diferenças:<ul><li>A API de perfil é uma chamada de API REST e o serviço de [!DNL Customer Attributes] usa FTP.</li><li>A API de perfil do Adobe Target envia dados somente para a Adobe Target em vez de para toda a CX Enterprise.</li><li>O serviço de [!DNL Customer Attributes] fornece uma interface simples para criar e gerenciar esses dados externos.</li></ul> |
