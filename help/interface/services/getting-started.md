...
title: Introdução ao CX Enterprise
index: true
...
## Implementar o Serviço de ID de visitante
...
Se você não estiver usando [!UICONTROL marcas Experience Platform], implemente o Serviço de ID de Visitante manualmente pela Implantação da JavaScript (`VisitorAPI.js`) da seguinte maneira:
...
Use o processo a seguir para garantir que o Serviço de ID de visitante seja implementado corretamente no site.

1. Apague os cookies do site para que você possa ver a solicitação para o Serviço de ID de visitante (a solicitação ocorre na primeira visita e, em seguida, uma vez por visitante, por semana).
...
1. Verifique se a solicitação do Analytics contém o parâmetro `mid` (a ECID). Durante o período de carência (se estiver habilitado), você também deve ver um parâmetro `aid` (a ID do visitante do Analytics).

Resposta esperada contendo a ECID:

![Resposta esperada contendo a ECID](../assets/mac_id_response.png)

Solicitação de imagem do Analytics contendo a ECID (também conhecida como `mid` ou _ID do visitante_):

![Solicitação de imagem do Analytics contendo a ECID](../assets/mid.png)

ECID na solicitação da mbox:

![ECID na solicitação da mbox](../assets/mbox_request.png)

### Qual é o período de carência?

Depois de implantar o Serviço de ID de visitante, os novos visitantes não receberão mais uma ID do Analytics (`aid`). Se as seções no site ainda não implementaram o Serviço de ID de visitante, quando os visitantes navegam até essas seções, a ECID (`mid`) não é reconhecida e os visitantes recebem uma ID do Analytics herdada (`aid`). Essa configuração pode causar possíveis problemas, incluindo visitas duplicadas e atribuições incorretas.

Por exemplo, se a seção de suporte do seu site for gerenciada em um CMS diferente, você terá um arquivo JavaScript do Analytics diferente para essa seção. Se você implantar a ECID no site principal antes de implantar o Serviço de ID de visitante no site de suporte, os novos visitantes receberão uma ID do Analytics herdada quando visitarem a seção de suporte. As visitas que abrangem ambas as seções do site são relatadas como visitas diferentes.

Implantar o Serviço de ID do visitante nos sites que usam vários arquivos JavaScript ou outras tecnologias (como Flash) pode causar problemas de coordenação. Esses problemas ocorrem porque você deve habilitar o Serviço de ID de visitante em todas as partes do site ao mesmo tempo. Ao configurar um período de carência, os novos visitantes continuam a receber uma ID de visitante do Analytics pelo Serviço de ID de visitante. Os visitantes podem ser identificados consistentemente em seções do seu site que não foram atualizadas para usar o Serviço de ID de visitante.
...
* (Opcional) Uma ou mais IDs. Dependendo da implementação do Serviço de ID de visitante, também é possível enviar uma ou mais IDs, como IDs do CRM ou endereços de email com hash. Se os dados forem enviados para o Adobe Analytics, serão transferidos para o Adobe Audience Manager. A Adobe não recomenda fornecer dados pessoais ao Adobe Analytics. Em vez disso, use um hash unidirecional para mascarar os dados antes de enviá-los para a Adobe.
* Segmentos originados no [!DNL Analytics] através do segmento back-end que compartilha o recurso.
* O cookie `demdex.net` será configurado se os cookies de terceiros não estiverem bloqueados. O cookie próprio `AMCV_###@AdobeOrg` sempre está configurado com o Serviço de ID do visitante.
